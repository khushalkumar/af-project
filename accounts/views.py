...
def form(request):
    if request.method == 'POST':
        ...
        ph = str(request.POST['phonenumber'])
        length = len(ph)
        if not(ph.startswith('7') or ph.startswith('8') or ph.startswith('9')):
            return render(request, 'accounts/user-form.html',
            {'error': "Phone number should start with 7 or 8 or 9 and should only have 10 digits..", 'first_name': request.POST['firstname']})

        if not (length == 10):
            return render(request, 'accounts/user-form.html',
            {'error': "Phone number should start with 7 or 8 or 9 and should only have 10 digits..", 'email':request.POST['email'], 'password1': request.POST['password1'], 'password2': request.POST['password2'] , 'first_name': request.POST['firstname'], 'last_name': request.POST['lastname'], 'dob': request.POST['date'], 'phone_number': request.POST['phonenumber']})
